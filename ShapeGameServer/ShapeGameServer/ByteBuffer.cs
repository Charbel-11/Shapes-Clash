...
namespace ShapeGameServer
{
    ...
    public class ByteBuffer : IDisposable
    {
        ...
        public byte[] ReadBytes(int length, bool MoveIndex = true)
        {
            ...
            if (readPos + length - 1 >= Buff.Count) {
                Console.WriteLine("You are at pos " + readPos + ", and you want " + length + " bytes. However, there is only " + Buff.Count + " bytes in total");
                throw new Exception("Not enough unread bytes available");
            }
